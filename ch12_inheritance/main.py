...
class Circle(Shape):
    def __init__(self, name, radius):
        super().__init__(name)
        self.radius = radius
        print(f"반지름: {self.radius}, {self.name} 이 생성되었습니다.")

    def area(self):
        return 3.14 * self.radius * self.radius

    def draw(self):
        print(f"{self.name}인 원의 넓이는 {self.area()} 입니다.")
...
class Rectangle(Shape):
    def __init__(self, name, width, height):
        ...
        self.width = width
        self.height = height
        print(f'height : {self.height}, width : {self.width}인 {self.name}이 생성됨')

    def area(self):
        return self.width * self.height

    def draw(self):
        print(f'{self.name}의 직사각형의 넓이는 {self.area()} 입니다.')
